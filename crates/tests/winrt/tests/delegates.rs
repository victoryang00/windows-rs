...
#[test]
fn non_generic() -> windows_core::Result<()> {
    type Handler = AsyncActionCompletedHandler;

    assert_eq!(Handler::IID, windows_core::GUID::from("A4ED5C81-76C9-40BD-8BE6-B1D90FB20AE7"));
...
#[test]
fn generic() -> windows_core::Result<()> {
    type Handler = TypedEventHandler<Uri, i32>;

    assert_eq!(Handler::IID, windows_core::GUID::from("DAE18EA9-FCF3-5ACF-BCDD-8C354CBA6D23"));
...
fn event() -> windows_core::Result<()> {
    ...
    set.MapChanged(MapChangedEventHandler::<windows_core::HSTRING, windows_core::IInspectable>::new(move |_, args| {
        let args = args.as_ref().unwrap();
        tx.send(true).unwrap();
        let set = set_clone.clone();
        let _: IObservableMap<windows_core::HSTRING, windows_core::IInspectable> = set.try_into().unwrap();
        assert!(args.Key()? == "A");
        assert!(args.CollectionChange()? == CollectionChange::ItemInserted);
        Ok(())
    }))?;

    set.Insert("A", windows_core::IInspectable::try_from(1_u32)?)?;
